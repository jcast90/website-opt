...
//old for loop
 /* var phases = [i];
  for (var i = 0; i < items.length; i++) {
    var phases =  Math.sin(scrollTopDefined + (i % 5));

    items[i].style.left = items[i].basicLeft + 100 * phases + 'px';
  } */

//new suggested for loop
  var phase = [];

for (var i = 0; i < 5; i++) {
    phase.push(Math.sin(scrollTopDefined + i) * 100);
}
